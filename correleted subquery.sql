...
-- select emp.employe_name from employee as emp join department as d on emp.department = d.department where 
-- emp.salary > d.salary
...
where emp.salary > ( select avg(ein.salary) from employee as ein where ein.department_name = emp.department_name);


select emp.ename,emp.salary,emp.department_name from employee as emp
where emp.salary = ( select min(ein.salary) from employee as ein where ein.department_name = emp.department_name);


-- cte ( common table expression )
with xyz as
(select department_name ,avg(salary) as salary from employee group by department_name)
select emp.ename,emp.salary,emp.department_name from employee as emp join xyz 
on emp.department_name=xyz.department_name where emp.salary > xyz.salary;
