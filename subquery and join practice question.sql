...
-- 1. find employees who earn more than the averange salary of all employees 
...
-- 2. find the employees who earn max salary 
select * from employees where salary = (select max(salary) from employees);

-- second highest salary and employee information 
select * from employees where salary = (
		select max(salary) from employees 
				where salary < (select max(salary) from employees));
                
-- 3. find employee who work in department located in IT or finance 
...
-- 4. find employees who has salary greater than all employees of hr department 
select dept_id from departments where dept_name = 'HR';

select * from employees 
where salary > (select max(salary) from employees where dept_id = (select dept_id from departments where dept_name = 'HR' ));

-- second method 
select salary from employees as e join departments as d on e.dept_id = d.dept_id and d.dept_name = 'HR';

select * from employees where 
salary >all ( select salary from employees as e join departments as d on e.dept_id = d.dept_id and d.dept_name = 'HR');

-- 5. find employees who earn the same salary as any employee 

with cte as (
select d.dept_id , salary from employees as e join departments as d on e.dept_id = d.dept_id and d.dept_name = 'Sales')

select * from employees where salary in (select salary from cte)
and dept_id not in (select dept_id from cte );


-- 6. find employees who earn more than the average salary of their own department 
...
-- find departments whose average salary is greater then the overall average salary of all employees 
select dept_id,avg(salary) from employees group by dept_id having avg(salary) > (select avg(salary) from employees);


-- find the departments with the maximum total salary 
...
-- find departments whose minimum salary is greater than the minimum salary of the sales department 
select dept_id , min(salary) from employees group by dept_id having min(salary) > (
select min(salary) from employees as e join departments as d on e.dept_id = d.dept_id and dept_name = 'Sales');
 -- second method 
 select dept_id , min(salary) from employees group by dept_id having min(salary) >any (
select salary from employees as e join departments as d on e.dept_id = d.dept_id and dept_name = 'Sales');